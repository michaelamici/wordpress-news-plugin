		...
		return useSelect(
			( select ) =>
				select( blocksStore ).getActiveBlockVariation(
					'kestrel-courier/juicy-headline-multi-query',
					attributes
				)?.allowedControls,
...
export const getTransformedBlocksFromPattern = (
	...
	while ( blocksQueue.length > 0 ) {
		...
		if ( block.name === 'kestrel-courier/juicy-headline-multi-query' ) {
	...
	return useSelect(
		( select ) => {
			const activeVariationName = select(
				blocksStore
			).getActiveBlockVariation( 'kestrel-courier/juicy-headline-multi-query', attributes )?.name;

			if ( ! activeVariationName ) {
				return 'kestrel-courier/juicy-headline-multi-query';
			}
			...
			const activePatterns = getPatternsByBlockTypes(
				`kestrel-courier/juicy-headline-multi-query/${ activeVariationName }`,
				rootClientId
			);
	...
	const { activeVariationName, blockVariations } = useSelect(
		( select ) => {
			...
			return {
				...
				blockVariations: getBlockVariations( 'kestrel-courier/juicy-headline-multi-query', 'block' ),
			};
		},
		[ attributes ]
	);
