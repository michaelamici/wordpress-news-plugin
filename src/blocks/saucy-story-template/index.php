 ...
 */
function kestrel_courier_block_core_post_template_uses_featured_image( $inner_blocks ) {
	foreach ( $inner_blocks as $block ) {
		...
		if ( $block->inner_blocks && kestrel_courier_block_core_post_template_uses_featured_image( $block->inner_blocks ) ) {
			return true;
		}
	}
...
function render_block_kestrel_courier_saucy_story_template( $attributes, $content, $block ) {
	...
	if ( kestrel_courier_block_core_post_template_uses_featured_image( $block->inner_blocks ) ) {
...
function register_block_kestrel_courier_saucy_story_template() {
	register_block_type_from_metadata(
		dirname( __DIR__, 3 ) . '/build/blocks/saucy-story-template',
		array(
